...
class WikipediaIndex:
    def __init__(self):
        path = os.path.join(
            DIR, "..", "data", "wikipedia-3sentence-level-retrieval-index", "files"
        )
        ...
        self._contriever = AutoModel.from_pretrained("facebook/contriever-msmarco").to(
            self._device
        )

        self._df_sentences = pd.read_parquet(wiki_sentence_path, engine="fastparquet")

        self._wiki_index = faiss.read_index(
            ...
            try:
                ...
                for j in range(1, w + 1):
                    if (
                        ...
                        text = (
                            self._df_sentences.iloc[indice - j]["text_snippet"] + text
                        )
                    else:
                        break
