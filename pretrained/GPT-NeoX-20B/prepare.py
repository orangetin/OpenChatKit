...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-name", type=str, default="EleutherAI/gpt-neox-20b", help="model-name"
    )
    parser.add_argument("--save-dir", type=str, default=DIR, help="model-name")
    parser.add_argument(
        "--offload-dir", type=str, default=None, help="directory to offload from memory"
    )
    ...
    for i in range(len(model.gpt_neox.layers)):
        ...
        torch.save(
            model.gpt_neox.layers[i].state_dict(),
            os.path.join(save_path, f"pytorch_{i}.pt"),
        )
