...
def load_decentralized_checkpoint(
    model, checkpoint_path, n_stages=2, n_layer_per_stage=14
):
    ...
    for i in range(n_stages):
        ...
        checkpoint = torch.load(
            os.path.join(input_path, f"prank_{i}_checkpoint.pt"),
        ...
        if i == 0:
            ...
            model.gpt_neox.final_layer_norm.weight.data[:] = _tmp[
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config-name", type=str, default="EleutherAI/gpt-neox-20b", help="config-name"
    )
    parser.add_argument("--ckpt-path", type=str, default=None, help="ckpt-path")
    parser.add_argument("--save-path", type=str, default=None, help="save-path")
    parser.add_argument("--n-stages", type=int, default=8, help="pipeline group size")
    parser.add_argument(
        "--n-layer-per-stage", type=int, default=6, help="n layers per GPU device"
    )
