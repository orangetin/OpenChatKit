...
def add_torch_distributed_arguments(parser):
    ...
    parser.add_argument(
        "--world-size", type=int, default=4, metavar="D", help="world-size (default: 4)"
    )
    parser.add_argument(
        "--pipeline-group-size",
        type=int,
    ...
    parser.add_argument(
        "--rank", type=int, default=0, metavar="N", help="rank of the node"
    )
...
def add_task_arguments(parser):
    parser.add_argument(
        "--train-data",
        nargs="+",
        default=["./glue_dataset/data/QQP/train.tsv"],
        metavar="S",
    ...
    parser.add_argument(
        "--valid-data",
        nargs="+",
        default=["./glue_dataset/data/QQP/test.tsv"],
        metavar="S",
        help="path to the training data",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--vocab-file",
        type=str,
        default="./glue_dataset/data/bert-large-cased-vocab.txt",
        metavar="S",
        help="which tokenizer to use.",
    )
    parser.add_argument("--vocab-extra-ids", type=int, default=0, metavar="N", help="-")
    parser.add_argument(
        "--make-vocab-size-divisible-by", type=int, default=128, metavar="N", help="-"
    )
    parser.add_argument("--optimizer", type=str, default="adamw", metavar="N", help="-")


def add_model_arguments(parser):
    parser.add_argument("--seq-length", type=int, default=1024, metavar="N", help="-")
    parser.add_argument("--embedding-dim", type=int, default=768, metavar="N", help="-")
    parser.add_argument("--num-layers", type=int, default=4, metavar="N", help="-")
    parser.add_argument("--num-heads", type=int, default=12, metavar="N", help="-")


def add_training_hyper_parameter_arguments(parser):
    parser.add_argument(
        "--train-log-backend", type=str, default="print", metavar="N", help="-"
    )
    parser.add_argument(
        "--project-name", type=str, default="test", metavar="N", help="-"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument("--lr", type=float, default=0.01, metavar="N", help="-")
    parser.add_argument("--num-iters", type=int, default=10, metavar="N", help="-")
...
def add_mixed_precision_arguments(parser):
    ...
    parser.add_argument(
        "--use-offload", action="store_true", help="Offload optim states to CPU"
    )
...
def get_dist_arguments_str(args, add_rank=True):
    dist_str = (
        ...
        + str(args.world_size)
        + "_p"
        + str(args.pipeline_group_size)
        + "_"
        + str(args.gradient_accumulate_step)
        + "_d"
        + str(args.data_group_size)
    )
    if add_rank:
