...
def load_checkpoint(pipe, args):
    ...
    checkpoint_step_path = os.path.join(
        args.checkpoint_path, f"checkpoint_{latest_step}"
    )
        ...
        pipe.model.model.load_state_dict(
            torch.load(
                os.path.join(
                    ...
                    f"prank_{get_pipeline_parallel_rank()}_checkpoint.pt",
                ),
    ...
    try:
        pipe.optimizer.load_state_dict(
            torch.load(
                os.path.join(
                    checkpoint_step_path,
                    f"prank_{get_pipeline_parallel_rank()}_optimizer.pt",
                ),
                map_location=torch.device("cpu"),
            )
        )
    ...
    try:
        pipe.scheduler.load_state_dict(
            torch.load(
                os.path.join(
                    checkpoint_step_path,
                    f"prank_{get_pipeline_parallel_rank()}_scheduler.pt",
                )
            )
        )
    except:
...
def save_stream_dataloader_state_dict(dataloader, pipe, args):
    ...
    torch.save(
        ...
        os.path.join(checkpoint_step_path, "dataset_state_dict.pt"),
    )
