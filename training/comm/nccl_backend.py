...
class NCCLCommunicator:
    def __init__(
        self, comm_rank: int, cuda_id: int, comm_group_size: int, comm_name: str
    ):
    ...
    def send(self, tensor: torch.Tensor, dst: int, stream=cupy.cuda.Stream.null):
    ...
    def recv(self, tensor: torch.Tensor, src: int, stream=cupy.cuda.Stream.null):
    ...
    def broadcast(self, tensor: torch.Tensor, src: int, stream=cupy.cuda.Stream.null):
        self.comm.bcast(
            tensor.data_ptr(),
            torch.numel(tensor),
            _type_torch_to_cupy(tensor.dtype),
    ...
    def scatter(
        ...
        if self.comm_rank == src:
            for i in range(self.comm_group_size):
                self.send(scatter_list[i], i, stream)
        self.recv(tensor, src, stream)
    ...
    def gather(
        ...
        if self.comm_rank == dst:
            for i in range(self.comm_group_size):
                self.recv(gather_list[i], i, stream)
        self.send(tensor, dst, stream)
        cupy.cuda.nccl.groupEnd()
    ...
    def all_to_all(
        ...
        assert (
            len(output_tensor_list) == self.comm_group_size
            and len(input_tensor_list) == self.comm_group_size
        )
        cupy.cuda.nccl.groupStart()
        for i in range(self.comm_group_size):
    ...
    def all_reduce_opt(
        ...
        for i in range(self.comm_group_size):
            self.comm.send(
                tensor.data_ptr() + i * chunk_size * element_size,
                chunk_size,
                t_type,
                i,
                stream.ptr,
