...
def train_loop(args, pipe, device, train_data_loader, test_data_loader):
    ...
    input_ids = torch.zeros([args.batch_size, args.seq_length], dtype=torch.int64).to(
        device
    )

    prefix_masks = torch.zeros(
        [args.batch_size, args.seq_length], dtype=torch.uint8
    ).to(device)
    ...
    if get_pipeline_parallel_rank() == 0 and dp_rank == 0:
        for i, data in enumerate(train_data_loader):
            ...
            pipe.sgd_iter(
                input_ids,
                labels,
                aux_input_data={"prefix_masks": prefix_masks},
                loss_func=gpt_loss_func,
            )

            if (
                args.evaluation_steps > 0
                and pipe.global_step % args.evaluation_steps == 0
            ):
...
def main():
    ...
    parser.add_argument(
        "--model-name", type=str, default="gpt2", metavar="S", help="model name or path"
    )
    parser.add_argument(
        "--tokenizer-name",
        type=str,
    ...
    parser.add_argument(
        "--model-type", type=str, default="gpt2", metavar="S", help="model name or path"
    )
    parser.add_argument("--checkpoint-path", type=str, default="model_checkpoints/gpt2")
    parser.add_argument(
        "--task-name", type=str, default="cot", metavar="S", help="task name"
    )
    ...
    parser.add_argument(
        "--evaluation-data", type=str, default=None, help="path of eval data in jsonl"
    )
    parser.add_argument(
        "--evaluation-num-batch",
        type=int,
        default=None,
        help="for debug purpose, only eval the first several batch.",
    )
    parser.add_argument(
        "--checkpoint-steps",
        type=int,
    ...
    parser.add_argument(
        "--net-interface", type=str, default="lo", metavar="S", help="net_interface"
    )
    parser.add_argument("--job-id", type=str, default="0", metavar="S", help="an uuid")
    ...
    if args.profiling == "no-profiling":
        ...
        trace_file = (
            prefix
            + get_learning_arguments_str(args)
            + get_model_arguments_str(args)
            + get_dist_arguments_str(args)
            + get_mixed_precision_arguments_str(args)
            + "_"
            + args.profiling
            + "_"
            + args.trace_postfix
