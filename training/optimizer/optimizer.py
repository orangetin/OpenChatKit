...
class Fp16Optimizer:
    ...
    def __init__(self, optimizer, grad_scaler, device, offload=False):
        ...
        if self.grad_scaler:
            self.found_inf = (
                torch.cuda.FloatTensor([0.0], device=device)
                if not self.offload
                else torch.FloatTensor([0.0])
            )

        self._dummy_overflow_buf = (
            torch.cuda.IntTensor([0], device=device)
            if not self.offload
            else torch.IntTensor([0])
        )
        ...
        for param_group in self.optimizer.param_groups:
            ...
            for i, param in enumerate(param_group["params"]):
                if param.requires_grad:
                    ...
                    if self.offload:
                        optimizer_param = (
                            param.detach().clone().float().to(device="cpu")
                        )
                ...
                if model_param.grad is not None:
                    if self.offload:
                        with torch.cuda.stream(self.gpu_to_cpu_stream):
                            optimizer_param.grad.copy_(
                                model_param.grad, non_blocking=False
                            )
                    else:
    ...
    def _unscale_optimizer_grads_and_check_for_nan(self):
        ...
        print(
            optimizer_grads[0].device,
            self.found_inf.device,
...
def get_fp16_optimizer(args, optimizer, device):
    ...
    return Fp16Optimizer(
        optimizer, grad_scaler, device, getattr(args, "use_offload", False)
    )
