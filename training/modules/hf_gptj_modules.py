...
def fixed_pos_embedding(x, seq_dim=1, seq_len=None):
    ...
    sinusoid_inp = torch.einsum(
        "i , j -> i j", torch.arange(seq_len, device=x.device), inv_freq
    ).float()
...
class GPTJMLP(_GPTJMLP):
    def __init__(
        self, intermediate_size, config, device="cpu"
...
class GPTJAttention(_GPTJAttention):
    def __init__(self, config, device="cpu"):
        ...
        self.k_proj = nn.Linear(
            self.embed_dim, self.embed_dim, bias=False, device=device
        )
    ...
    def forward(
        ...
        attn_output, attn_weights = self._attn(
            query, key, value, attention_mask, head_mask, prefix_masks=prefix_masks
        )
...
class GPTBlock(_GPTJBlock):
    def __init__(self, config, *args, use_checkpoint=True, device="cpu", **kargs):
        ...
        self.ln_1 = nn.LayerNorm(
            config.n_embd, eps=config.layer_norm_epsilon, device=device
        )
        ...
        def block_forward(
            x: torch.Tensor, attention_mask: torch.Tensor, prefix_masks: torch.Tensor
        ) -> torch.Tensor:
            res = x
            x = self.ln_1(x)
            x_a = self.attn(
                x, prefix_masks=prefix_masks, attention_mask=attention_mask
            )[0]
            x_m = self.mlp(x)
            return res + x_a + x_m
