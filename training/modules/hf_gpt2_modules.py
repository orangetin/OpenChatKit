...
class GPTAttention(_GPT2Attention):
    def _attn(
        self, query, key, value, attention_mask=None, head_mask=None, prefix_masks=None
    ):
        ...
        if not self.is_cross_attention:
            ...
            if prefix_masks is not None:
                ...
                causal_mask = causal_mask.repeat(
                    bsz, 1, 1, 1
                )  # (bsz, 1, src_len, tgt_len)
                causal_mask = causal_mask.permute(
                    0, 3, 1, 2
...
class GPTModel(_GPT2Model):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList(
            [
                GPTBlock(config, layer_idx=i, use_checkpoint=True)
