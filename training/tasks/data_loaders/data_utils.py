...
class UL2RProcessor:
    ...
    def preprocess_tokens_nlg(self, tokens):
        ...
        tokens = left + right
        tokens = tokens[: self.seq_length]
        tokens = tokens + (self.seq_length - len(tokens)) * [
    ...
    def preprocess_tokens_nlu(self, tokens):
        ...
        for i, chunk in enumerate(chunks):
            ...
            if (
                i in indices
                and not last_corrupt
                and extra_id_count < len(self.extra_ids)
            ):
...
class StreamDataset(IterableDataset):
    ...
    def __init__(
        self, data, tokenizer, seq_length=1024, doc_separator=None, cycling=True
    ):
        self.data = data
        self.tokenizer = tokenizer
        self.seq_length = seq_length
                ...
                if p < th:
                    ...
                    if SHOW_DATA:
                        if global_i % self.print_sample_every_n == 0:
                            ...
                            print(
                                f"**{task_name}**:",
                                self.tokenizer.decode(inputs["input_ids"]),
                            )
...
def name_to_dataset(task, tokenizer, args):
    if "prosocial_plus_regular.jsonl" in task:
        ...
        data = load_dataset(
            "json", data_files=task, split="train", streaming=True
        ).shuffle(buffer_size=100_000, seed=args.seed)
...
def get_train_data_loader(args, tokenizer, num_workers=1, state_dict=None):
    ...
    stream_dataset = StreamDatasetList(
        task_names, datasets, probs, tokenizer=tokenizer, seq_length=args.seq_length
    )
