...
class GpipeAsync:
    ...
    def __init__(
        ...
        self.post_node_rank = (
            self.pp_rank + 1 if self.pp_rank != self.pipeline_group_size - 1 else -1
        )
        ...
        self.forward_recv_ready_events = [
            torch.cuda.Event(enable_timing=self.enable_tidy_profiling, blocking=False)
            for _ in range(self.micro_batch_num)
        ]
        self.forward_comp_ready_events = [
            torch.cuda.Event(enable_timing=self.enable_tidy_profiling, blocking=False)
            for _ in range(self.micro_batch_num)
        ]

        self.backward_recv_ready_events = [
            torch.cuda.Event(enable_timing=self.enable_tidy_profiling, blocking=False)
            for _ in range(self.micro_batch_num)
        ]
        self.backward_comp_ready_events = [
            torch.cuda.Event(enable_timing=self.enable_tidy_profiling, blocking=False)
            for _ in range(self.micro_batch_num)
        ]
        ...
        if self.enable_tidy_profiling:
            self.profiling_log = []
            self.forward_recv_start_events = [
                torch.cuda.Event(enable_timing=True, blocking=False)
                for _ in range(self.micro_batch_num)
            ]
            self.forward_comp_start_events = [
                torch.cuda.Event(enable_timing=True, blocking=False)
                for _ in range(self.micro_batch_num)
            ]
            self.forward_send_start_events = [
                torch.cuda.Event(enable_timing=True, blocking=False)
                for _ in range(self.micro_batch_num)
            ]
            self.forward_send_end_events = [
                torch.cuda.Event(enable_timing=True, blocking=False)
                for _ in range(self.micro_batch_num)
            ]

            self.backward_recv_start_events = [
                torch.cuda.Event(enable_timing=True, blocking=False)
                for _ in range(self.micro_batch_num)
            ]
            self.backward_comp_start_events = [
                torch.cuda.Event(enable_timing=True, blocking=False)
                for _ in range(self.micro_batch_num)
            ]
            self.backward_send_start_events = [
                torch.cuda.Event(enable_timing=True, blocking=False)
                for _ in range(self.micro_batch_num)
            ]
            self.backward_send_end_events = [
                torch.cuda.Event(enable_timing=True, blocking=False)
                for _ in range(self.micro_batch_num)
            ]
        ...
        if self.pp_rank == 0:
            ...
            self.input_micro_batches = [
                torch.zeros(
                    (self.micro_batch_size, self.seq_length, self.embedding_dim),
                    requires_grad=do_train,
                    device=self.device,
                    dtype=self.dtype,
                )
                for _ in range(self.micro_batch_num)
    ...
    def _compute_micro_batch_size(self):
        micro_batch_float_num = (
            self.micro_batch_size * self.seq_length * self.embedding_dim
        )
        if self.use_fp16:
            print(
                "=======Current micro-batch send/recv size: {} MB (fp16)".format(
                    micro_batch_float_num * 2 // 1024 // 1024
                )
            )
        else:
            print(
                "=======Current micro-batch send/recv size: {} MB (fp32)".format(
                    micro_batch_float_num * 4 // 1024 // 1024
                )
            )
    ...
    def get_ts(self, event):
        return self.init_time_stamp + self.init_event.elapsed_time(event) * 1e3
            ...
            if self.pipeline_group_size > 1:
                if self.pp_rank == 0:  # Only send output to next node, do not receive
                    ...
                    with torch.cuda.stream(self.torch_comp_stream):
                        ...
                        current_micro_output = self.model(
                            self.input_micro_batches[i],
                            input_ids=input_ids_micro_batches[i],
                            **{k: v[i] for k, v in aux_input_data.items()},
                        )
                        self.torch_comp_stream.record_event(
                            self.forward_comp_ready_events[i]
    ...
    def profiling_forward_stage(self):
        ...
        for i in range(self.micro_batch_num):
            if self.pp_rank != 0:
                recv_slot = (
                    self.forward_recv_start_events[i].elapsed_time(
                        self.forward_recv_ready_events[i]
                    )
                    * 1e3
                )
                recv_log = {
                    ...
                    "tid": "1. forward-recv",
                    "ts": self.get_ts(self.forward_recv_start_events[i]),
                    "dur": recv_slot,
                    "args": {"micro-batch": i},
                    "cname": "startup",
                }  # cname is for color, a little silly.
                # print(recv_log)
                self.profiling_log.append(recv_log)

            comp_slot = (
                self.forward_comp_start_events[i].elapsed_time(
                    self.forward_comp_ready_events[i]
                )
                * 1e3
            )
            comp_log = {
                ...
                "ph": "X",
                "pid": self.global_rank,
                "tid": "2. forward-compute",
                "ts": self.get_ts(self.forward_comp_start_events[i]),
                "dur": comp_slot,
                "args": {"micro-batch": i},
                "cname": "good",
            }
            ...
            if self.pp_rank != self.pipeline_group_size - 1:
                send_slot = (
                    self.forward_send_start_events[i].elapsed_time(
                ...
                send_log = {
                    ...
                    "pid": self.global_rank,
                    "tid": "3. forward-send",
                    "ts": self.get_ts(self.forward_send_start_events[i]),
                    "dur": send_slot,
                    "args": {"micro-batch": i},
    ...
    def backward_stage(
        self,
        cached_output_micro_batches: List[torch.Tensor],
        target=None,
        ...
        for i in range(self.micro_batch_num):
            if self.pipeline_group_size > 1:
                if (
                    self.pp_rank == self.pipeline_group_size - 1
                ):  # only send grad back to last node, do not receive
                    with torch.cuda.stream(self.torch_comp_stream):
                    ...
                    self.pp_rank == 0
                ):  # only receive grad from previous node, do not send
                    with torch.cuda.stream(self.torch_recv_stream):
                        cupy_recv_stream = cupy.cuda.ExternalStream(
                            self.torch_recv_stream.cuda_stream
                        )
                        ...
                        self.comm.recv(
                            self.output_micro_batches_grad[i],
    ...
    def profiling_backward_stage(self):
        ...
        for i in range(self.micro_batch_num):
            if self.pp_rank != self.pipeline_group_size - 1:
                recv_slot = (
                    self.backward_recv_start_events[i].elapsed_time(
                        self.backward_recv_ready_events[i]
                    )
                    * 1e3
                )
                recv_log = {
                    ...
                    "pid": self.global_rank,
                    "tid": "4. backward-recv",
                    "ts": self.get_ts(self.backward_recv_start_events[i]),
                    "dur": recv_slot,
                    "args": {"micro-batch": i},
                    "cname": "startup",
                }
            ...
            comp_slot = (
                self.backward_comp_start_events[i].elapsed_time(
                    self.backward_comp_ready_events[i]
                )
                * 1e3
            )
            ...
            if self.pp_rank != 0:
                send_slot = (
                    self.backward_send_start_events[i].elapsed_time(
                ...
                send_log = {
                    ...
                    "pid": self.global_rank,
                    "tid": "6. backward-send",
                    "ts": self.get_ts(self.backward_send_start_events[i]),
                    "dur": send_slot,
                    "args": {"micro-batch": i},
                    "cname": "thread_state_iowait",
                }
    ...
    def profiling_optimizer_step(self):
        ...
        if not self.use_dp:
            optimizer_slot = (
                self.optimizer_start_event.elapsed_time(self.optimizer_end_event) * 1e3
            )
            optimizer_log = {
                "name": "opt",
                "ph": "X",
                "pid": self.global_rank,
                "tid": "7. optimizer-step",
                "ts": self.get_ts(self.optimizer_start_event),
                "dur": optimizer_slot,
                "cname": "bad",
            }
            # print(optimizer_log)
            ...
            self.profiling_log.extend(
                self.dp_optim.profiling_data_parallel(
    ...
    def sgd_iter(
        ...
        if self.use_fp16 and self.use_dynamic_scale:
            scales_buffer = [
                torch.ones_like(self.optimizer.grad_scaler._scale)
                for _ in range(self.pipeline_group_size)
            ]
            self.comm.all_gather(self.optimizer.grad_scaler._scale, scales_buffer)
            self.optimizer.grad_scaler._scale.data[:] = min(
                [s.item() for s in scales_buffer]
            )
        ...
        print(
            "Rank {} node forward pass {}/{} takes {:3.2f}s".format(
                self.global_rank, step, self.gradient_accumulate_step, forward_slot
            )
        )
        ...
        print(
            "Rank {} node backward pass {}/{} takes {:3.2f}s".format(
                self.global_rank,
                step,
        ...
        if step == self.gradient_accumulate_step - 1:
            ...
            end_time = time.time()
            print(
                "Rank {} node optimizer step takes {:3.2f}s".format(
        ...
        print(
            "Rank {} node whole iteration takes {:3.2f}s".format(
    ...
    def infer_stage(
        self, input_data=None, aux_input_data=None, labels=None, pred_func=None
    ):
        if aux_input_data is not None:
            for k in aux_input_data:
                aux_input_data[k] = torch.chunk(
            ...
            if self.pipeline_group_size > 1:
                if self.pp_rank == 0:  # Only send output to next node, do not receive
                    ...
                    with torch.cuda.stream(self.torch_comp_stream):
                        ...
                        current_micro_output = pred_func(
                            current_micro_output, labels[i]
                        )
                        self.torch_comp_stream.record_event(
                            self.forward_comp_ready_events[i]
                        )
