...
class CentralPSDP:
    def __init__(
        ...
        self.broadcast_reduced_gradients_ready_event = torch.cuda.Event(
            enable_timing=self.enable_tidy_profiling, blocking=False
        )
        ...
        print(
            "Total number of parameters: {}, element size: {}, total size {} MB.".format(
                num_paras, element_size, num_paras * element_size // 1024 // 1024
            )
        )
        ...
        if self.flatten:
            ...
            print(
                "Flattened parameter number: {}, element size: {}.".format(
                    self.flatten_para.data.numel(),
                    self.flatten_para.data.element_size(),
                )
            )
            print(
                "Flattened parameter grad number: {}, element size: {}.".format(
                    self.flatten_para.grad.numel(),
                    self.flatten_para.grad.element_size(),
                )
            )
        ...
        if self.enable_tidy_profiling:
            ...
            if self.flatten:
                self.reduce_gradients_start_event = torch.cuda.Event(
                    enable_timing=True, blocking=False
                )
                self.reduce_gradients_end_event = torch.cuda.Event(
                    enable_timing=True, blocking=False
                )
                self.broadcast_reduced_grad_start_event = torch.cuda.Event(
                    enable_timing=True, blocking=False
                )
            else:
                ...
                for name, _ in self.module.named_parameters():
                    self.reduce_gradients_start_events[name] = torch.cuda.Event(
                        enable_timing=True, blocking=False
                    )
                    self.reduce_gradients_end_events[name] = torch.cuda.Event(
                        enable_timing=True, blocking=False
                    )
                    self.broadcast_reduced_grad_start_events[name] = torch.cuda.Event(
                        enable_timing=True, blocking=False
                    )
                    self.broadcast_reduced_grad_end_events[name] = torch.cuda.Event(
                        enable_timing=True, blocking=False
                    )
        ...
        if self.enable_tidy_profiling:
            if name is None:
                ...
                self.dp_comm_stream.record_event(
                    self.reduce_gradients_start_events[name]
                )
    ...
    def profile_mark_broadcast_start(self, name=None):
        if self.enable_tidy_profiling:
            if name is None:
                self.dp_comm_stream.record_event(
                    self.broadcast_reduced_grad_start_event
                )
            else:
                self.dp_comm_stream.record_event(
                    self.broadcast_reduced_grad_start_events[name]
                )
    ...
    def profile_mark_broadcast_end(self, name=None):
        if self.enable_tidy_profiling:
            if name:
                self.dp_comm_stream.record_event(
                    self.broadcast_reduced_grad_end_events[name]
                )
    ...
    def _reduce_gradients(self):
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            if self.flatten:
                ...
                self.dp_comm.reduce(
                    self.flatten_para.grad, dst=0, stream=cupy_dp_stream
                )
        ...
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            if self.flatten:
                ...
                self.dp_comm.broadcast(
                    self.flatten_para.grad, src=0, stream=cupy_dp_stream
                )
            ...
            self.dp_comm_stream.record_event(
                self.broadcast_reduced_gradients_ready_event
            )
    ...
    def profiling_data_parallel(self, init_time_stamp, init_event):
        ...
        if self.flatten:
            reduce_slot = (
                self.reduce_gradients_start_event.elapsed_time(
                    self.reduce_gradients_end_event
                )
                * 1e3
            )
            reduce_log = {
            ...
            for name, para in self.module.named_parameters():
                reduce_slot = (
                    self.reduce_gradients_start_events[name].elapsed_time(
                        self.reduce_gradients_end_events[name]
                    )
                    * 1e3
                )
                reduce_log = {
                    ...
                    "pid": self.global_rank,
                    "tid": "7. optimizer-comm",
                    "ts": self.get_ts(self.reduce_gradients_start_events[name]),
                    "dur": reduce_slot,
                    "cname": "cq_build_passed",
                    "args": {"para": name, "size": torch.numel(para.data)},
                }
                # print(reduce_log)
                profiling_log.append(reduce_log)

        optimizer_slot = (
            self.optimizer_step_start_event.elapsed_time(
        ...
        if self.flatten:
            broadcast_slot = (
                self.broadcast_reduced_grad_start_event.elapsed_time(
                    self.broadcast_reduced_gradients_ready_event
                )
                * 1e3
            )
            ...
            for name, para in self.module.named_parameters():
                broadcast_slot = (
                    self.broadcast_reduced_grad_start_events[name].elapsed_time(
                        self.broadcast_reduced_grad_end_events[name]
                    )
                    * 1e3
                )
                broadcast_log = {
                    ...
                    "pid": self.global_rank,
                    "tid": "7. optimizer-comm",
                    "ts": self.get_ts(self.broadcast_reduced_grad_start_events[name]),
                    "dur": broadcast_slot,
                    "cname": "cq_build_passed",
                    "args": {"para": name, "size": torch.numel(para.data)},
                }
