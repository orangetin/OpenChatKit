...
class LocalDP:
    def __init__(
        ...
        self.allreduce_gradients_start_event = torch.cuda.Event(
            enable_timing=self.enable_tidy_profiling, blocking=False
        )
        self.allreduce_grad_ready_event = torch.cuda.Event(
            enable_timing=self.enable_tidy_profiling, blocking=False
        )
        ...
        if self.enable_tidy_profiling:
            ...
            if self.flatten:
                ...
                for name, _ in self.module.named_parameters():
                    self.allreduce_gradients_start_events[name] = torch.cuda.Event(
                        enable_timing=True, blocking=False
                    )
                    self.allreduce_gradients_end_events[name] = torch.cuda.Event(
                        enable_timing=True, blocking=False
                    )
        ...
        if self.enable_tidy_profiling:
            if name is None:
                ...
                self.dp_comm_stream.record_event(
                    self.allreduce_gradients_start_events[name]
                )
    ...
    def profile_mark_allreduce_end(self, name=None):
        if self.enable_tidy_profiling:
            if name:
                self.dp_comm_stream.record_event(
                    self.allreduce_gradients_end_events[name]
                )
    ...
    def optimizer_step(self):
        ...
        with torch.cuda.stream(self.torch_optim_comp_stream):
            self.torch_optim_comp_stream.record_event(
                self.allreduce_gradients_start_event
            )
    ...
    def profiling_data_parallel(self, init_time_stamp, init_event):
        ...
        if self.flatten:
            allreduce_slot = (
                self.allreduce_gradients_start_event.elapsed_time(
                    self.allreduce_grad_ready_event
                )
                * 1e3
            )
            allreduce_log = {
            ...
            for name, para in self.module.named_parameters():
                allreduce_slot = (
                    self.allreduce_gradients_start_events[name].elapsed_time(
                        self.allreduce_gradients_end_events[name]
                    )
                    * 1e3
                )
                allreduce_log = {
                    ...
                    "pid": self.global_rank,
                    "tid": "7. optimizer-comm",
                    "ts": self.get_ts(self.allreduce_gradients_start_events[name]),
                    "dur": allreduce_slot,
                    "cname": "cq_build_passed",
