...
def flatten_params(param_set, chunk=None):
    ...
    grads = [
        p.grad.data if p.grad is not None else torch.zeros_like(p.data) for p in params
    ]
    ...
    flatten_weights_tensor = torch.zeros(total_size, dtype=weights[0].dtype).to(
        weights[0].device
    )
    ...
    for i in range(len(params)):
        flatten_weights_tensor[offset : offset + weights[i].numel()] = weights[
            i
        ].reshape(-1)
