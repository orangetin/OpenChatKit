...
class ShardedPSDP:
    def __init__(
        ...
        print(
            "Flattened parameter number: {}, element size: {}.".format(
                self.flatten_para.data.numel(), self.flatten_para.data.element_size()
            )
        )
        print(
            "Flattened parameter grad number: {}, element size: {}.".format(
                self.flatten_para.grad.numel(), self.flatten_para.grad.element_size()
            )
        )
    ...
    def _sync_gradients(self):
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            self.dp_comm.all_reduce_opt(
                self.flatten_para.grad, self.grad_buffer, stream=cupy_dp_stream
            )
    ...
    def profiling_data_parallel(self, init_time_stamp, init_event):
        ...
        assert self.flatten
        allreduce_slot = (
            self.sync_gradients_start_event.elapsed_time(
